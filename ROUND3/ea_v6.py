...
# ea_v4: exclude near-ATM bleeders (5200/5300) from Engine B MM
MM_STRIKES      = [4000, 4500, 5000, 5100, 5400, 5500, 6000, 6500]
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # Engine A — ea_v6: disabled (high variance on day 2; net uncertain on IMC)
        try:
            vt = VelvetTrader(state, last_wall_mid=trader_data.get("vev_wall_mid"))
            # Only persist wall_mid for smile fit; don't trade the underlying
        ...
        # Engine B — one VoucherTrader per strike (ea_v4: skip 5200/5300 bleeders)
        for k in MM_STRIKES:
            try:
